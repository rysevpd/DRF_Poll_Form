...
class PollById(APIView):
    ...
    def post(self, request, id):
        try:
            ...
            if CompletedTest.objects.filter(userId=userId, poll=poll).count() > 0:
            ...
            submis = CompletedTest(userId=userId, poll=poll)
            submis.save()
...
class PollsByUser(APIView):
    def get(self, request, id):
        try:
            result = []
            for submission in CompletedTest.objects.filter(userId=id).order_by('submitTime'):
                submissionDict = CompletedTestSerializer(submission).data
